...
def test_create_month(client):
    ...
    r = client.post("/months", json={"year": 2025, "month": 1}, headers=headers(token))
...
def test_prev_total_isolation_between_groups(client):
    ...
    r = client.post("/months", json={"year": 2025, "month": 1}, headers=headers(token1))
    month1_id = r.json()["id"]
    client.post(f"/months/{month1_id}/charges",
    ...
    r = client.post("/months", json={"year": 2025, "month": 1}, headers=headers(token2))
    month2_id = r.json()["id"]
    client.post(f"/months/{month2_id}/charges",
    ...
    r = client.post("/months", json={"year": 2025, "month": 2}, headers=headers(token1))
    assert r.status_code == 201
    detail = client.get(f"/months/{r.json()['id']}", headers=headers(token1)).json()
...
def test_two_groups_can_share_same_year_month(client):
    ...
    r1 = client.post("/months", json={"year": 2025, "month": 5}, headers=headers(token1))
    assert r1.status_code == 201

    token2 = register(client, "melissa")
    create_group(client, token2)
    r2 = client.post("/months", json={"year": 2025, "month": 5}, headers=headers(token2))
...
def test_duplicate_month_in_same_group(client):
    ...
    token = register(client, "nicolas")
    create_group(client, token)
    client.post("/months", json={"year": 2025, "month": 3}, headers=headers(token))
    r = client.post("/months", json={"year": 2025, "month": 3}, headers=headers(token))
